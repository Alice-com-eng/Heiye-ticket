...
import org.opengoofy.index12306.biz.ticketservice.common.exception.SeatOccupiedRetryException;
    ...
    public void lockSeat(String trainId, String departure, String arrival,
        ...
        for (TrainPurchaseTicketRespDTO each : trainPurchaseTicketRespList) {
            ...
            SeatDO updateSeatDO = SeatDO.builder()
                    ...
                    .build();
            int rows = seatMapper.update(updateSeatDO, updateWrapper);
            if (rows == 0) {
                throw new SeatOccupiedRetryException("座位已被抢");
            }
        }
    }

    /**
     * 解锁：只解锁"出发站→到达站"这一条区间记录
     */
    @Override
